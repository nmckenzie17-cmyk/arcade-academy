(function () {
  ...
  function askQuestion(turnPlayer,callback){activeQuestion=MultiplayerQuestionHelper.next();if(!activeQuestion){showFeedback('No questions available','wrong');return;}modalBox.style.setProperty('--chip-color',turnPlayer.color);modalWho.textContent=`${turnPlayer.name.toUpperCase()} — ANSWER TO EARN YOUR MOVE`;modalBody.innerHTML=`<div class="prompt"></div><div class="answer-list"></div>`;modalBody.querySelector('.prompt').textContent=activeQuestion.prompt;activeQuestion.answers.forEach(answer=>{const button=document.createElement('button');button.className='mc-option';button.textContent=answer.text;button.dataset.correct=String(answer.correct);button.onclick=()=>{const buttons=modalBody.querySelectorAll('.mc-option');buttons.forEach(b=>{b.disabled=true;if(b.dataset.correct==='true')b.classList.add('correct');});if(!answer.correct)button.classList.add('incorrect');setTimeout(()=>{closeModal();callback(answer.correct);},answer.correct?350:2000);};modalBody.querySelector('.answer-list').appendChild(button);});modalOverlay.classList.add('show');}
