(function () {
  ...
  function render(){const blueprint=dotReward('board')?.id==='dot-n-box-deducer_blueprint_board',matrix=dotReward('theme')?.id==='dot-n-box-deducer_deduction_matrix',tic=dotReward('crossover')?.id==='dot-n-box-deducer_tic_tac_grid',prism=dotReward('lines')?.id==='dot-n-box-deducer_prismatic_lines',neon=dotReward('dots')?.id==='dot-n-box-deducer_neon_dots';ctx.clearRect(0,0,600,600);ctx.fillStyle=blueprint?'#082b55':matrix?'#020b12':'#0b0e1a';ctx.fillRect(0,0,600,600);if(blueprint||matrix){ctx.strokeStyle=blueprint?'rgba(95,205,255,.13)':'rgba(35,255,166,.1)';ctx.lineWidth=1;const gap=blueprint?24:30;for(let n=0;n<=600;n+=gap){ctx.beginPath();ctx.moveTo(n,0);ctx.lineTo(n,600);ctx.stroke();ctx.beginPath();ctx.moveTo(0,n);ctx.lineTo(600,n);ctx.stroke();}}for(let r=0;r<state.dotsY-1;r++)for(let c=0;c<state.dotsX-1;c++){const box=state.boxes[r][c],pos=dotPos(r,c),size=state.layout.cellSize;if(box.owner!==null){const owner=state.players[box.owner];ctx.fillStyle=hexToRgba(owner.color,.18);ctx.fillRect(pos.x+4,pos.y+4,size-8,size-8);ctx.fillStyle=owner.color;ctx.font='700 11px Lexend';ctx.textAlign='center';ctx.fillText(tic?(box.owner===0?'X':'O'):`P${box.owner+1}`,pos.x+size/2,pos.y+size/2);}}ctx.lineCap='round';Object.keys(state.lines).forEach((key,index)=>{const line=state.lines[key],[a,b]=lineEndpoints(key);ctx.strokeStyle=line.drawn?(prism?`hsl(${(performance.now()/20+index*13)%360} 95% 65%)`:state.players[line.owner].color):'rgba(138,148,190,.18)';ctx.lineWidth=line.drawn?6:4;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();});for(let r=0;r<state.dotsY;r++)for(let c=0;c<state.dotsX;c++){const{x,y}=dotPos(r,c);ctx.fillStyle=neon?`hsl(${(performance.now()/18+(r+c)*35)%360} 100% 72%)`:'#eef2ff';if(neon){ctx.shadowColor=ctx.fillStyle;ctx.shadowBlur=12;}ctx.fillRect(x-(neon?5:4),y-(neon?5:4),neon?10:8,neon?10:8);ctx.shadowBlur=0;}state.particles.forEach(p=>{ctx.fillStyle=hexToRgba(p.color,Math.max(p.life,0));ctx.fillRect(p.x-3,p.y-3,6,6);});}
